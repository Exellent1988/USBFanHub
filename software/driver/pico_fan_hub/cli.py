...
def cmd_status(args):
    ...
    for i in range(6):
        ...
        print(f"  {i+1}    | {rpms[i]:5d}  | {pwms[i]:3d}   | {duty_percent:5.1f}% {status}")
...
def cmd_rgb(args):
    ...
    if mode == 'off':
        ...
        if not args.color:
            print("Fehler: --color benötigt für static-Modus (z.B. --color 255,255,255)")
            hub.disconnect()
            return 1
...
Beispiele:
  ...
  pico-fan-ctl rgb off                   # RGB aus
    ...
    parser_set_pwm.add_argument('fan', type=int, choices=range(1, 7), help='Lüfter-Nummer (1-6)')
    parser_set_pwm.add_argument('duty', type=int, help='PWM-Wert (0-255)')
    
    # set-all-pwm
    parser_set_all = subparsers.add_parser('set-all-pwm', help='Setze PWM für alle Lüfter')
    parser_set_all.add_argument('duty', type=int, help='PWM-Wert (0-255)')
    
    # rgb
    parser_rgb = subparsers.add_parser('rgb', help='RGB-Steuerung')
    parser_rgb.add_argument('mode', choices=['off', 'rainbow', 'breathing', 'static'], help='RGB-Modus')
